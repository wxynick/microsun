 ...
 * <tt>decode(
 * String s, boolean gzipCompressed )</tt>. Added the ability to "suspend"
 * encoding in the Output Stream so you can turn on and off the encoding if you
 * need to embed base64 data in an otherwise "normal" stream (like an XML file).
 * </li>
	...
	private static int decode4to3(byte[] source, int srcOffset,
		...
		// Example: DkLE
			...
			try {
				...
				log.error(String.valueOf(source[srcOffset]) + ": "
						+ DECODABET[source[srcOffset]]);
				log.error(String.valueOf(source[srcOffset + 1]) + ": "
						+ DECODABET[source[srcOffset + 1]]);
				log.error(String.valueOf(source[srcOffset + 2]) + ": "
						+ DECODABET[source[srcOffset + 2]]);
				log.error(String.valueOf(source[srcOffset + 3]) + ": "
